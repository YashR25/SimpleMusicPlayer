package com.example.simplemusicplayer.data
...
class ContentResolverHelper @Inject constructor(@ApplicationContext val context: Context) {
    ...
    private fun getCursorData(): MutableList<Audio>{
        ...
        //SQL Statement ("SELECT projection FROM MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
        // WHERE selectionClause = selectionArg ORDER BY sortOrder")
        mCursor = context.contentResolver.query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
            projection,
            selectionClause,
            selectionArg,sortOrder)

        //get columns indexes to access values of rows from table
        mCursor?.use {cursor ->
            ...
            cursor.apply {
                if (count == 0){
                    ...
                    //access values from table row by row using cursor
